...
export function PerformerAssignedView() {
  const { activeSharedOrderId, pendingOrder, resetOrderFlow } = useDashboardStore();
  ...
  if (!sharedOrder?.performerName) return null;

  const eta = pendingOrder
    ? `${new Date(pendingOrder.scheduledDate).toLocaleDateString("ru-RU", { day: "numeric", month: "long" })} в ${pendingOrder.scheduledTime}`
    : "";
    ...
    <div className="max-w-sm mx-auto px-4 pt-8 pb-10">
      ...
      >
        ...
        <div className="p-6 flex items-center gap-4">
          ...
          >
            {sharedOrder.performerAvatar ?? sharedOrder.performerName.slice(0, 2).toUpperCase()}
          </motion.div>
          <div>
            <p className="text-lg font-bold text-gray-900">{sharedOrder.performerName}</p>
            <div className="flex items-center gap-1 mt-0.5">
              <Star size={13} className="text-amber-400 fill-amber-400" />
              <span className="text-sm font-semibold text-gray-800">{sharedOrder.performerRating}</span>
              {sharedOrder.performerJobsCompleted != null && (
                <span className="text-sm text-gray-400">· {sharedOrder.performerJobsCompleted} заказов</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 flex flex-col gap-3">
          {sharedOrder.performerPhone && (
            <a
              href={`tel:${sharedOrder.performerPhone}`}
          ...
          {sharedOrder.performerTelegram && (
            <a
              href={`https://t.me/${sharedOrder.performerTelegram.replace("@", "")}`}
              target="_blank"
