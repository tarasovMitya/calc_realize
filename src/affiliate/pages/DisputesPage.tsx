    ...
    <div className="p-6">
      ...
      {isLoadingOrders ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
          <p className="text-gray-500 text-sm">Открытых споров нет</p>
        </div>
      ) : (
            ...
            <div key={o.id} className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900">{o.serviceName ?? o.categoryName}</p>
                  <p className="text-sm text-gray-500 mt-0.5">Исполнитель: {o.performerName}</p>
                  <p className="text-sm text-gray-500">Адрес: {o.address}</p>
                  <p className="text-sm text-gray-700 font-medium mt-1">{formatPrice(o.priceTotal)}</p>
                  {o.disputeComment && (
